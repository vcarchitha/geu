$(document).ready(function() {
        ...
        var ugmandateLabels;
        var timerInterval, resendTimerInterval, partialTimer, partialResendTimer;
        var formIdSet;
    ...
    $(validationInputs).on('change focusin focus focusout keypress keydown keyup mousedown mouseup', function() {
        clearInterval(partialTimer, partialResendTimer);
    ...
    function getOTP() {
        $.ajax({
            type: "POST",
            url: "/bin/geu/loginAdmissionForm",
        ...
        $(".form-error-message, .invalid-error-message, .custom-error-message").text("");
            ...
            if ($("#fname").val().length == 0 && $("#lname").val().length == 0 && $("#email").val().length == 0 && $("#mobile").val().length == 0) {
                ...
                $("#sendOtp").click(function() {
                    ...
                    resendTimerInterval =  setInterval(function() { resendTimer(); }, 1000);           
                });
                var sec = 4;
                function Timer() {
                    $('#hideMsg').show();
    ...
    /*function registrationProcessApply() {
     ...
     function registrationProcessPartailApply() {
        var isFormValid = true;
        $('#selection-mode-submit').on('click', function() {
            if($("#selection-mode-form input[type='radio']:checked").val() == "registration") {
                $(".upload-registration-wrapper").show();
            ...
            else if($("#selection-mode-form input[type='radio']:checked").val() != '') {
    ...
    function getBasicDetails() {
        $.ajax({
            type: "POST",
            url: "/bin/geu/loginAdmissionForm",
            data: $('#registration-form').serialize() +  '&type=applyFresh' + '&formAction=verifyotp' + '&otpcode=' + $("#enterOtp").val() + '&phoneNumber=' + $('#mobile').val(),
            success: function(msg) {
                if (msg == "invalid_otp") {
    ...
    $("#formNumber").on('keypress focus, keypress, keydown, keyup, mousedown, mouseup', function() {
        clearInterval(timerInterval, resendTimerInterval);
    ...
    function selectionMode() {
        $.ajax({
            type: "POST",
            async: false,
            url: "/bin/geu/selectionModeServlet",
                    ...
                    for (var key in json) {
                        if (json.hasOwnProperty(key)) {
                            ...
                            if (key == "Name") {
                               ...
                               var pucResultDeclare = json[key];
                                $('input:radio[name="resultsDeclared_12th"]').filter('[value="' + pucResultDeclare + '"]').prop('checked', true);
                                /*if(pucResultDeclare == "Yes") {
                                    $('#resultsDeclared_12th').prop("checked", "true");
                                }
                                else if(pucResultDeclare == "No") {
								...
								$("#level option[value='" + json[key] + "']").prop('selected', 'selected');
                                loadProgramList(); 
                                if(programName != '') {
                                    $("#course_preference_1 option[value='" + programName + "']").prop('selected', 'selected');
                                }
                                if($('#level').val() == "Post-Graduate Programmes") {
                                    ugmandateLabels = $(".graduation-wrapper label, .choose-level label, .graduation-wrapper .form-group.select-option .heading").not(".graduation-wrapper .select-option label");
    ...
    $("#partiallyFilledForm").click(function(e) {
        ...
        if ($("#formNumber").val().length == 0) {
            ...
            else if($("#enterOtpPartially").val().length != '6'){
                $(".partially-filled-form-error-message").text("");
                return false;
                isFormValid = false;
            }
    ...
    //$('input:radio[name="resultsDeclared_12th"]').prop('readOnly');
	...
	function getFeeDetails(){
        ...
        var ugResultsDeclared = $('input[name="resultsDeclaredGraduation"]:checked').val();
		var ispcmorjeemandatory = $('#course_preference_1').find('option:selected').attr('ispcmorjeemandatory');
		var selectedProgram = $('#course_preference_1').val();
		$.ajax({
            type: "POST",
            url: "/bin/geu/levelProgramList",
            data: 'resourcePath=' + resourcePath + '&programSelected=' + selectedProgram +  '&type=fee' + '&pcm=' + pcm + '&jee=' + jee + '&gender=' + gender + '&sslcPercentage=' + sslcAggregate + '&hscPercentage=' + hscPercentage + '&hscResultsDeclared=' + hscResultsDeclared +'&ugPercentage=' + ugPercentage +'&ugResultsDeclared=' + ugResultsDeclared +'&levelSelected=' + $('#level').val() + '&isPcmOrJeeMandatory=' + ispcmorjeemandatory,
            success: function(msg) {
				if(msg == "[]") {
    ...
    function loadProgramList() {  
        ...
        $.ajax({
            ...
            type: "POST",
            url: "/bin/geu/levelProgramList",
            data: 'resourcePath=' + resourcePath + '&levelSelected=' + level+ '&type=program',
            success: function(msg) {
                var json = jQuery.parseJSON(msg);
    ...
    $("#level").change(function(e) {
        ...
        hscmandateLabels = $(".hsc-wrapper label").not(".hsc-wrapper .select-option label, .hsc-wrapper .pcmPercentage label, .hsc-wrapper .jeeScore label");
        ugmandateLabels = $(".graduation-wrapper label, .graduation-wrapper .form-group.select-option .heading").not(".graduation-wrapper .select-option label");

        if($('#level').val() == "Under-Graduate Programmes") {
            $('.graduation-wrapper').hide();
            $(".graduation-wrapper input, .graduation-wrapper select").prop("disabled", true);
            $(hscmandateLabels).addClass("required");
            $(uploadHscSectionLabel).addClass("required");
            $(ugmandateLabels).removeClass("required");
        }
        else {
            $(uploadHscSectionLabel).removeClass("required");
        }

        if($('#level').val() == "Post-Graduate Programmes") {
            $('.graduation-wrapper').show();
            $(".graduation-wrapper input, .graduation-wrapper select").prop("disabled", false);
    ...
    //Label Star Mark
    var mandateLabels = $(".choose-level label, .program-wrapper label, .sslcBoard label, .sslcPassingYear label, .sslcMarks label, .sslcPercentage label, .sslc-wrapper .form-group.select-option .heading, .pucBoard label, .pucSchool label, .pucStatus label, .pucPassingYear label, .hsc-wrapper .form-group.select-option .heading, .pucCity label, .pucPincode label, .pucPercentage label, .dob label, .aadharNo label, .communication-address .address-heading, .address-heading .heading, .fatherName label, .motherName label");
    ...
    $("#save").click(function() {
        ...
        var isUgValid = true;
        var isRegValid = true;
        var validationInputs = $("#board_10th, #year_of_passing_10th, #marking_system_10th, #aggregate_marks_10th, #board_12th, #intermediate_senior, #status_12th, #year_of_passing_12th, #projected_marks_12th, #city_12th, #pincode_12th, #date_of_birth, #aadharNo, #correspondence_address, #permanent_address, #father_name, #mother_name, #level, #course_preference_1");
        var ugInputs = $("#higher_education_institute, #grduationUniversityName, #graduationCourse, #graduationSpecialisation, #year_of_graduation, #cityGraduation, #pincodeGraduation, #cgpa_percentage");
        var element,ugElement;

        //$('#basic--details-error-message').html('');
        $(ugInputs).each(function() {
            ugElement = $(this);
            if($("#level").val() == "Post-Graduate Programmes") {
                if (ugElement.val() == "") {
                    ...
                    scrollToBasic();
                    //$("#basic--details-error-message").html($("<li>").text("Please Enter Graduation Details"));
        ...
        $(validationInputs).each(function() {
            ...
            if (element.val() == "") {
                ...
                $(".basic-details-form-error-message").text("Please Enter Mandatory Fields");
                //$("#basic--details-error-message").add($("<li>").text("Please Enter Mandatory Fields"));
                scrollToBasic();
                isInputValid = false;
            } else if ( this.value.trim() !== '' ) {
        ...
        if($('#aadharNo').val() != "" && $('#aadharNo').val().length != "12") {
            ...
            //$("#basic--details-error-message").html($("<li>").text("Please enter valid aadhaar No"));
            return false;
        }
        ...
        if($("#selection-mode-form input[type='radio']:checked").val() == "registration") {
            if (($("#registrationMarksheet").val().length === 0 && $("#registration-marksheet-name").text().length === 0)) {
                ...
                //$("#basic--details-error-message").html($("<li>").text("Please Upload Latest Marksheet"));
                return false;
            }
        }
        ...
        if ($('.no-course-found .heading').text() != '') {
            ...
            //$("#basic--details-error-message").append($("<li>").text("Please Upload Latest Marksheet"));
            isFormValid = false;
            scrollToBasic();
        } 
        ...
        if($('#course_preference_1').find('option:selected').attr('ispcmorjeemandatory') == 1) {
            if ($("#pcmPercentage").val() == '' && $("#jeeScore").val() == '') {
                $(".basic-details-form-error-message").text("Please Enter PCM or JEE Score");
                //$("#basic--details-error-message").html($("<li>").text("Please Enter PCM or JEE Score"));
                isFormValid = false;
                scrollToBasic();
            }
            else {
        ...
        if($('#jeeScore').val() > 360 ) {
          ...
          //$("#basic--details-error-message").html($("<li>").text("Please Enter Valid JEE Score"));
        ...
        if ($("#father_contact").val() == '' && $("#mother_contact").val() == '') {
            ...
            scrollToBasic(); 
            //$("#basic--details-error-message").html($("<li>").text("Please enter Father or Mother Mobile Number"));
            isFormValid = false;
        }
            ...
            scrollToBasic(); 
            //$("#basic--details-error-message").html($("<li>").text("Please enter father or mother valid mobile number"));
            isFormValid = false;
        }
        ...
        if ($('input.copyAddress').is(':checked')) { 
            $('input.copyAddress').val('on');
        }
        else {
            $('input.copyAddress').val('off');
        }
        ...
        else if(element.val() == '' && ugElement.val() == '') {
            return isInputValid;
            return isUgValid;
        }
        else if($("#level").val() == "Post-Graduate Programmes" && element.val() != '' && ugElement.val() == '') {
            return isUgValid;
        }
        
        if (isFormValid == true && isInputValid == true && isUgValid == true) {
            var formData;
			var eligibility = $('#eligibility').text().replace('%', '');
            ...
            //var dataToPass =  '&totalFees=' + totalFees + '&duration=' + duration +'&formId=' + $('#formId').text() + '&copyAddress=' +$('input[name="copyAddress"]').val();
            $.ajax({
                type: "POST",
                url: "/bin/geu/basicAdmissionForm",
                //data: $('#basic-details-form').serialize() + '&eligibility=' + encodeURIComponent($('#eligibility').text()) + '&totalFees=' + totalFees + '&discountRate=' + discountRate + '&discountedFees=' + discountedFees + '&duration=' + duration +'&formId=' + $('#formId').text() + '&copyAddress=' +$('input[name="copyAddress"]').val(),
                data: formData,
                processData: false,
                contentType: false,
                success: function(msg) {
    ...
    $("#uploadSubmit").click(function() {
        ...
        if (($("#uploadPhoto").val().length === 0 && $("#photoWrapper #photo-name").text().length === 0) || ($("#uploadSslcMarksheet").val().length === 0 && $("#sslcWrapper #sslc-marksheet-name").text().length === 0) || ($("#uploadHscMarksheet").val().length === 0 && $(".upload-wrapper.HscMarksheet #hsc-marksheet-name").text().length === 0)) {
            scrollTopUpload();
            $(".upload-error-message").text("Please Upload Mandatory Documents");
            isFormValid = false;
        }
        /*else if (($(".essay-question-first textarea").val().length === 0 || $(".essay-question-second textarea").val().length === 0)) {
            scrollTopUpload();
        ...
        if ($('input.videoWhatsApp').is(':checked')) { 
            $('input.videoWhatsApp').val('on');
        }
        else {
            $('input.videoWhatsApp').val('off');
        }
        ...
        if(isFormValid) {
            ...
            if (window.FormData !== undefined) {
                ...
                $.ajax({
                    url: '/bin/geu/uploadAdmissionForm',
                    data: formData,
                    contentType: false,
                    processData: false,
                    type: 'POST',
    ...
    $('#paymentSubmitRegistration').on('click', function(e) {
        ...
        if($('input[name="payment-mode"]:checked').length <= 0) {
            ...
            $.ajax({
                ...
                success: function(responseText) {
                    window.location.href = '/content/geu/en/offline-success.html';
                },
            });
            $('.form-tabs ul li').removeClass('selected');
            $("#myLinks li:last-child").addClass("changebg");
