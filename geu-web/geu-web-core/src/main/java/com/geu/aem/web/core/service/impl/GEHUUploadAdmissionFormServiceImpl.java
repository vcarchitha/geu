...
import com.geu.aem.web.core.service.GEHUUploadAdmissionFormService;
import com.geu.aem.web.service.SMSApplicationService;

@Component(service = GEHUUploadAdmissionFormService.class, immediate = true)
public class GEHUUploadAdmissionFormServiceImpl implements GEHUUploadAdmissionFormService{
	
	private final Logger LOGGER = LoggerFactory
			.getLogger(GEHUUploadAdmissionFormServiceImpl.class);
	...
	public void updateUploadGEHUAdmissionData(Map<String, Object> uploadAdmissionMap,
			...
			if (StringUtils.isNotBlank(updateUploadQuery)) {
				...
				if(uploadAdmissionMap.get(ResourceConstants.FORM_ID)!=null){
					...
					if(updateStatus==1){
						...
						gehuEmailService.sendSubmissionEmailToUser(email);
						...
						gehuEmailService.sendAdmissionDataToAdmin(formId, admissionMap, admissionFormIDMap, fileNameMap);
	...
	private Map<String, String> getMobileEmailFromFormID(String formID){
		...
		Connection connection = null;
		try{
			String phNumberQuery = "SELECT contactNum, email FROM gehu_admission WHERE formId = '"
					+ formID + "'";
			connection = getConnection();
	...
	private Map<String, String> getAdmissionDataFromFormID(String formID){
		...
		try{
			String getDetailsQuery = "SELECT * FROM gehu_admission WHERE formId = '"
					+ formID + "'";
	...
	private Map<String, InputStream> getBlobFromAdmission(String formID){
		...
		try{
			String getDetailsQuery = "SELECT photo,  marksheet_10th, "
					...
					+ " FROM gehu_admission WHERE formId = '"+formID + "'";
	...
	private Map<String, String> getFileNames(String formID){
		...
		try{
			String getDetailsQuery = "SELECT photo_name,  marksheet_10th_name,"
					...
					+ " FROM gehu_admission WHERE formId = '"
					+ formID + "'";
			LOGGER.info("getDetailsQuery::::" + getDetailsQuery);
			connection = getConnection();
			PreparedStatement checkPreparedStmt = connection
