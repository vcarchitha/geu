...
@Component(service = Servlet.class, property = {
	"service.description=" + "Selection Mode Servlet",
	"sling.servlet.methods=" + HttpConstants.METHOD_POST,
	"sling.servlet.paths=" + "/bin/geu/selectionModeServlet"})
public class SelectionModeServlet extends SlingAllMethodsServlet {
	
	@Reference
	SelectionModeService modeService;
	
	private static final long serialVersionUID = 1L;
	private final Logger LOGGER = LoggerFactory
			.getLogger(SelectionModeServlet.class);
		...
		try{
			...
			if(StringUtils.isNotBlank(selectionMode)){
				...
				if(StringUtils.isNotBlank(formId)){
					modeService.updateSelectionMode(selectionMode, formId);
				}			
			}else{
