...
@Component(service = Servlet.class, property = {
		"service.description=" + "GEHU Login Admission Form Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_POST,
		"sling.servlet.paths=" + "/bin/gehu/loginAdmissionForm" })
public class GEHULoginAdmissionFormServlet extends SlingAllMethodsServlet {

	@Reference
	GEHULoginAdmissionFormService gehuAdmissionService;
	...
	private final Logger LOGGER = LoggerFactory
			.getLogger(GEHULoginAdmissionFormServlet.class);

	protected void doPost(SlingHttpServletRequest request,
			SlingHttpServletResponse response) {
		LOGGER.info("Inside GEHULoginAdmissionFormServlet!!!!");
		try{		
			...
			if(type.equalsIgnoreCase(ResourceConstants.ADMISSION_APPLYFRESH)){
				...
				if (requestMap != null) {
					...
					boolean userIfExists = gehuAdmissionService.checkUserAlreadyExistsInGEHU(admissionMap);
					if(userIfExists){
						...
						if(StringUtils.isNotBlank(formAction)){
							if(formAction.equals("sendotp")){
								...
								LOGGER.info("OTP while APPLY FRESH : SEND OTP CASE: GEHU: "+otp);
								...
								if (!isOTPValid){
									...
									String phoneNumber=admissionMap.get(ResourceConstants.CONTACT_NUMBER);
									gehuAdmissionService.insertGEHULoginAdmissionData(admissionMap, phoneNumber);
									jsonObject = gehuAdmissionService.getGEHULoginAdmissionData(admissionMap);
				...
				if(StringUtils.isNotBlank(formID)){
					boolean formIDCheck = gehuAdmissionService.checkFormIDAlreadyExistsInGEHU(formID);
					if(!formIDCheck){
						String userNotFound = ResourceConstants.FORM_ID_DOESNOT_EXISTS_ERROR;
						response.getWriter().write(userNotFound);
					}else{
						boolean paymentStatus = gehuAdmissionService.checkPaymentStatus(formID);
						if(paymentStatus){
							String formAction = request.getParameter(ResourceConstants.FORM_ACTION);
							if(StringUtils.isNotBlank(formAction)){
								if(formAction.equals("sendotp")){
									...
									String contactNum = gehuAdmissionService.getGEHUMobileNumberFormID(formID);
									...
									if (!isOTPValid){
										...
										JSONObject jsonObject = new JSONObject();
										jsonObject = gehuAdmissionService.getGEHUAdmissionDataFromFormID(formID);
