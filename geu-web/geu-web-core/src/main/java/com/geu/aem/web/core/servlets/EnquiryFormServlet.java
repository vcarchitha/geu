...
property = { "sling.servlet.paths=" + "/bin/geu/enquiryform",
		"sling.servlet.methods=" + "POST"})
public class EnquiryFormServlet extends SlingAllMethodsServlet {

	private static final long serialVersionUID = 1L;
	final static Logger LOGGER = LoggerFactory.getLogger(EnquiryFormServlet.class);
	...
	private void sendEnquiryMail(String toAddress) {
		...
		try {
        String jsonPath=ResourceConstants.EMAIL_CONFIG+"enquiry-form.json";
		...
		awsEmailService.sendEmail(subject, multipart, toAddress);
			...
			LOGGER.error("Messaging Exception in  EnquiryFormServlet" + e.getMessage());
		}catch (NullPointerException e) {
			 LOGGER.error("NullPointerException Exception in  EnquiryFormServlet" + e.getMessage()); 
		}
